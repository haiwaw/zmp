...
int main() 
{
   ...
   XGpioPs_SetDirectionPin(&mio_resets, MIO_7, 1);
   XGpioPs_SetOutputEnablePin(&mio_resets, MIO_7, 1);
	XGpioPs_WritePin(&mio_resets, MIO_7, 0x0);
	for (Delay = 0; Delay < RESET_DELAY; Delay++);
	XGpioPs_WritePin(&mio_resets, MIO_7, 0x1);
	// End PHY Reset
...

   {
      ...
      status = GpioOutputExample(XPAR_AXI_GPIO_3_DEVICE_ID,5);
