...
namespace Socket
{
    ...
    template <>
    inline int UDP::send<string>(Ip ip, Port port, string data)
    {
        return this->send<char>(ip, port, data.c_str(), data.length() + 1);
    }
    
    template <>
    inline int UDP::send<string>(Address address, string data)
    {
    ...
    template <>
    inline Datagram<string> UDP::receive<string>(void)
    {
        Datagram<string> ret;
